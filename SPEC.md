...
What IS in scope:
- A **best-effort** `attempt_full_clear(state, budget) -> ClearResult` implemented via the same bounded/beam search as §9.3, clearly documented as a heuristic approximation, not a guarantee. `ClearResult` carries:
  - `found_full_clear: bool` — `True` only if a real sequence clearing all cells was found (a certificate).
  - `certificate: list[Move] | None` — the actual move sequence, if found.
  - `moves_used: int | None` — length of that sequence, when found (the headline efficiency number per §9's move-count objective).
  - `best_apples_cleared: int` — the highest apples-cleared count observed across all explored branches, populated regardless of whether a full clear was found. This is what answers "how many apples can we clear from this board" even when a full clear isn't found or doesn't exist.
  - If `found_full_clear` is `False`, that means "search budget exhausted without finding a full clear" — **not** a proof of unsolvability. This distinction must be documented on the function and honored by anything that consumes `ClearResult` (e.g., the CLI, §5.3 FR19).
