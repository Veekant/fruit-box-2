"""Tests for the game engine: scoring, state reporting, terminal, load, reset.
...
FR2 -- legality itself -- is deliberately *not* covered here: the rule lives on
``BoardState.is_legal`` and is tested in ``test_board.py``. ``GameEngine`` has no
legality code of its own; it delegates the check and the cell clearing to
``BoardState.apply_move``, so the tests here only pin down what this layer adds
-- score updates and exception propagation.
"""
